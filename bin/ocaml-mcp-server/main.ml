...
let accept_loop ~env ~sw ~config ~net addr =
  let listening_socket = Eio.Net.listen ~sw ~backlog:5 ~reuse_addr:true net addr in
  ...
  while true do
    ...
    Logs.info (fun m -> m "Accepted connection from %a" Eio.Net.Sockaddr.pp client_addr);
    (* Handle each client in a new fiber *)
    Eio.Fiber.fork ~sw (fun () ->
      try
        let transport = Mcp_eio.Socket.create_from_socket socket in
        let connection = Mcp_eio.Connection.create (module Mcp_eio.Socket) transport in
        handle_client ~sw ~env ~config connection;
        Logs.info (fun m -> m "Client disconnected: %a" Eio.Net.Sockaddr.pp client_addr)
      with exn ->
        (* Log error but don't crash the server *)
        Logs.err (fun m -> m "Client error (%a): %s" 
...
let project_root =
  let doc = "Project root directory. If not specified, will try to auto-detect." in
...
let setup_log =
  ...
  Term.(const setup_logging $ Fmt_cli.style_renderer ~docs () $ Logs_cli.level ~docs ())
...
let server_cmd =
  ...
  let man = [
    ...
    `P "ocaml-mcp-server is a Model Context Protocol (MCP) server that provides \
        tools for OCaml development, including module signature inspection and \
        Dune build status integration.";
    `S "TRANSPORT MODES";
    ...
    `I ("dune/build-status", "Get current build status from dune, including errors and warnings");
    `I ("ocaml/module-signature", "Get the signature of an OCaml module from build artifacts");
    `S "ENVIRONMENT VARIABLES";
    `I ("$(b,MCP_DEBUG)", "Enable debug logging (equivalent to $(b,--verbosity=debug))");
  ...
  let term =
    Term.(const (fun setup_log project_root stdio socket_port pipe_path enable_dune no_dune ->
      setup_log;
    ...
    ) $ setup_log $ project_root $ stdio $ socket_port $ pipe_path $ enable_dune $ no_dune)
  in
  Cmd.v info term
