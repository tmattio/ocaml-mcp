...
let default_conf =
  ...
  match
    Conf.default
    |> Conf.update_value ~name:"module-item-spacing" ~value:"compact"
  with
  | Ok conf -> (
...
type t = { conf : Ocamlformat_lib.Conf.t; mutex : Eio.Mutex.t }

let create () = { conf = default_conf; mutex = Eio.Mutex.create () }
  ...
  Eio.Mutex.use_ro t.mutex (fun () ->
      try
        ...
        let try_format format_type =
          Translation_unit.parse_and_format format_type ~input_name:path
            ~source:content conf
        in
      ...
      with exn ->
        Error (Printf.sprintf "Formatting error: %s" (Printexc.to_string exn)))
...
let format_type t ~typ =
  Eio.Mutex.use_ro t.mutex (fun () ->
      try
        let open Ocamlformat_lib in
        match
          Translation_unit.parse_and_format Core_type ~input_name:"<type>"
            ~source:typ t.conf
        with
        | Ok formatted -> Ok formatted
        | Error e ->
            let msg =
              Format.asprintf "%a"
                (Translation_unit.Error.print ~debug:false ~quiet:false)
                e
            in
            Error (`Msg msg)
