...
let evaluate_code env project_root code =
  ...
  try
    ...
    let clean_output s =
      ...
      let lines = String.split_on_char '\n' s in
      (* Filter each line *)
      let filtered = List.filter_map (fun line ->
        (* Check various patterns to skip *)
        if String.trim line = "" then None
        else if String.trim line = "#" then None  
        else if String.length line >= 13 && String.sub line 0 13 = "OCaml version" then None
    ...
    if String.length stderr > 0 then
      Ok (Tool_result.error stderr)
    else
      Ok (Tool_result.text (clean_output stdout))
  with
...
let handle env project_root args _ctx =
  evaluate_code env project_root args.code
