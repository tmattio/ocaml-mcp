...
let name = "fs/edit"
let description = "Replace text within a file. For OCaml files (.ml/.mli), automatically formats the result and returns diagnostics."
  ...
  let rec replace acc pos =
    try
      ...
      if idx + String.length old_string <= String.length text then
        ...
        acc ^ String.sub text pos (String.length text - pos)
    with Not_found ->
      acc ^ String.sub text pos (String.length text - pos)
  in
  replace "" 0
...
let handle merlin_client ocamlformat_client args _ctx =
  let { file_path; old_string; new_string; expected_replacements } = args in
  ...
  if not (Sys.file_exists file_path) then
    ...
    try
      ...
      if occurrences = 0 then
        Ok (Tool_result.error (Printf.sprintf "Failed to edit: could not find the string to replace in %s" file_path))
      else if occurrences <> expected then
        Ok (Tool_result.error (Printf.sprintf "Failed to edit: expected %d replacements but found %d in %s" expected occurrences file_path))
      else
        (* Perform replacement *)
        let new_content = replace_all content old_string new_string in
        ...
        let final_content, format_result =
          if is_ocaml_file then
            ...
            match Ocamlformat_client.format_doc ocamlformat_client ~path:file_path ~content:new_content with
        ...
        if is_ocaml_file then
          ...
          | Ok diagnostics ->
              ...
              let text_description = 
                ...
                if diag_count > 0 then
                  Printf.sprintf "Edited %s (%d replacements, %s, %d diagnostics)" file_path occurrences format_status diag_count
                else
                  Printf.sprintf "Edited %s (%d replacements, %s, no diagnostics)" file_path occurrences format_status
              in
          ...
          | Error err ->
              ...
              Ok (Tool_result.structured ~text:(Printf.sprintf "Edited %s (%d replacements, Merlin error: %s)" file_path occurrences err) result_json)
        else
          (* For non-OCaml files *)
          Ok (Tool_result.text (Printf.sprintf "Successfully modified %s (%d replacements)" file_path occurrences))
          
    with
    | Sys_error err -> Ok (Tool_result.error (Printf.sprintf "Error editing file: %s" err))
        ...
        `Assoc [
          ...
          ("properties", `Assoc [
            ...
            ("expected_replacements", `Assoc [
              ("type", `String "integer");
              ("description", `String "Number of replacements expected (defaults to 1)")
            ])
          ]);
