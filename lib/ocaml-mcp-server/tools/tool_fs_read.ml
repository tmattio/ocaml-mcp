...
let handle _sw env merlin_client project_root args _ctx =
...
let register server ~sw ~env ~merlin_client ~project_root =
  Server.tool server name ~description
                    ...
                    ( "offset",
                      `Assoc
                        [
                          ...
                          ( "description",
                            `String
                              "Optional: The 1-based line number to start \
                               reading from" );
                        ] );
                    ( "limit",
                      `Assoc
    ...
    (handle sw env merlin_client project_root)
